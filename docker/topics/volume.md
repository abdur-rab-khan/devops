...
- [Volumes in Docker](#volumes-in-docker)
  ...
  - [Command Reference](#command-reference)
    ...
    - [Creating Volume using Docker Compose](#creating-volume-using-docker-compose)
...
### Creating Volume using Docker Compose

- You can define and use volumes in a Docker Compose file using the `volumes` key, which is defined at the same level as `services`.
...
- In this example:
  ...
  - A volume named `my_volume` is created using the `local` driver.
  - The volume is mounted to `/path/in/container` inside the `my_service` container.

- At the top level of the Compose file, we define the volume under the `volumes` key,`my_volume` is the name of the volume, and then we specify options like `driver`, `driver_opts`, and `labels`.
- You can also use bind mounts in Docker Compose by specifying the host path directly in the `volumes` section of the service.

- **Options for volumes**

1. `driver`: Specify the volume driver to use (default is "local").

   - `local`: Default driver that stores data on the host filesystem.
   - `nfs`: Driver for NFS volumes.

2. `driver_opts`: Set additional options for the volume driver.
   ...
   - Example for NFS driver:
     ...
     driver_opts:
       type: "nfs"
       addr: <nfs_server_address>
       device: ":/path/on/nfs"
     ```

3. `external`: Specify whether the volume is external (i.e., not managed by Docker Compose).

   - Set to `true` if the volume already exists and should not be created by Docker Compose.
   - Example:
...
4. `labels`: Set metadata for the volume.
...
5. `name`: Specify a custom name for the volume.
   ...
   - Also specify using `environment` variable:

     ```yaml
     name: ${VOLUME_NAME:-DEFAULT_VOLUME_NAME} # "DEFAULT_VOLUME_NAME" use if VOLUME_NAME is not set
     ```
